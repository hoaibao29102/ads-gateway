...
export default function Admin() {
  ...
  const setActive = (h, val) => {
    const d = clone(data);
    (d.hosts[h] ||= { active: "A", links: {} }).active = val;
    setData(d);
  };
  const setLink = (h, key, url) => {
    const d = clone(data);
  ...
  const addHost = () => {
    ...
    const d = clone(data);
    if (d.hosts[h]) alert("Domain đã tồn tại");
    else d.hosts[h] = { active: "A", links: {} };
    setData(d); setNewHost("");
  };
  const removeHost = (h) => {
    const d = clone(data); delete d.hosts[h]; setData(d);
  };
  ...
  const save = async () => {
    ...
    alert(r.ok ? "Saved!" : "Save failed"); if (r.ok) load();
  };
